...
import './BitMath.sol';
import './FullMath.sol';
import '../base/ERC721/utils/Strings.sol';
import "./SafeMath.sol";
...
library NFTDescriptor {
    ...
    struct ConstructTokenURIParamsOne {
        uint256 tokenId;
    ...
    struct ConstructTokenURIParamsTwo {
        bool flipRatio;
        int24 tickLower;
        int24 tickUpper;
        int24 tickCurrent;
    ...
    function constructTokenURI(ConstructTokenURIParamsOne memory paramsOne, ConstructTokenURIParamsTwo memory paramsTwo)
        public 
        pure 
        returns (string memory) {
        string memory name = generateName(paramsOne, paramsTwo, feeToPercentString(paramsTwo.fee));
        string memory descriptionPartOne =
            generateDescriptionPartOne(
                escapeQuotes(paramsOne.quoteTokenSymbol),
                escapeQuotes(paramsOne.baseTokenSymbol),
                addressToString(paramsTwo.poolAddress)
            );
        string memory descriptionPartTwo =
            generateDescriptionPartTwo(
                paramsOne.tokenId.toString(),
                escapeQuotes(paramsOne.baseTokenSymbol),
                addressToString(paramsOne.quoteTokenAddress),
                addressToString(paramsOne.baseTokenAddress),
                feeToPercentString(paramsTwo.fee)
            );
        string memory image = Base64.encode(bytes(generateSVGImage(paramsOne, paramsTwo)));
    ...
    function generateName(ConstructTokenURIParamsOne memory paramsOne,
    ConstructTokenURIParamsTwo memory paramsTwo,
     string memory feeTier)
        private
        pure
        returns (string memory)
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    ' - ',
                    tickToDecimalString(
                        !paramsTwo.flipRatio ? paramsTwo.tickLower : paramsTwo.tickUpper,
                        paramsTwo.tickSpacing,
                        paramsOne.baseTokenDecimals,
                        paramsOne.quoteTokenDecimals,
                        paramsTwo.flipRatio
                    ),
                    '<>',
                    tickToDecimalString(
                        !paramsTwo.flipRatio ? paramsTwo.tickUpper : paramsTwo.tickLower,
                        paramsTwo.tickSpacing,
                        paramsOne.baseTokenDecimals,
                        paramsOne.quoteTokenDecimals,
                        paramsTwo.flipRatio
    ...
    function generateSVGImage(ConstructTokenURIParamsOne memory paramsOne,
    ConstructTokenURIParamsTwo memory paramsTwo) 
    internal 
    pure 
    returns (string memory svg) {
        NFTSVG.SVGParamsOne memory _svgParamsOne =
            NFTSVG.SVGParamsOne({
                quoteToken: addressToString(paramsOne.quoteTokenAddress),
                baseToken: addressToString(paramsOne.baseTokenAddress),
                poolAddress: paramsTwo.poolAddress,
                quoteTokenSymbol: paramsOne.quoteTokenSymbol,
                baseTokenSymbol: paramsOne.baseTokenSymbol,
                tokenId: paramsOne.tokenId,
                feeTier: feeToPercentString(paramsTwo.fee)
            });

        NFTSVG.SVGParamsTwo memory _svgParamsTwo = NFTSVG.SVGParamsTwo({
                tickLower: paramsTwo.tickLower,
                tickUpper: paramsTwo.tickUpper,
                tickSpacing: paramsTwo.tickSpacing,
                overRange: overRange(paramsTwo.tickLower, paramsTwo.tickUpper, paramsTwo.tickCurrent)
            });

        //working
        string memory _mantle = NFTSVG.generateSVGCardMantle(_svgParamsOne.quoteTokenSymbol , _svgParamsOne.baseTokenSymbol , _svgParamsOne.feeTier );
        string memory _curve = NFTSVG.generageSvgCurve(_svgParamsTwo.tickLower, _svgParamsTwo.tickUpper, _svgParamsTwo.tickSpacing, _svgParamsTwo.overRange);
        string memory _sparke = NFTSVG.generateSVGRareSparkle(_svgParamsOne.tokenId, _svgParamsOne.poolAddress);
        string memory _borderText = NFTSVG.generateSVGBorderText(_svgParamsOne.quoteToken, _svgParamsOne.baseToken, _svgParamsOne.quoteTokenSymbol, _svgParamsOne.baseTokenSymbol);
        return NFTSVG.generateSVG(_mantle, _curve, _sparke, _borderText);
